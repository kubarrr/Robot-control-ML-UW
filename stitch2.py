...
def get_new_shape_dst(dst_img, src_img, offset_x, offset_y):
    ...
    print(src_img.shape)
    new_shape[-offset_y:height_dst-offset_y, :, :] = dst_img[:, :, :]
    return new_shape
...
def calc_cost(pixel1, pixel2):
    # pixel1_gray=(pixel1*np.array([0.3, 0.59, 0.11])).sum()
    # pixel2_gray=(pixel2*np.array([0.3, 0.59, 0.11])).sum()
    # grayscale=np.abs(pixel1_gray-pixel2_gray).sum()
    diff = np.abs(pixel1 - pixel2)
    grayscale = 0.3 * diff[0] + 0.59 * diff[1] + 0.11 * diff[2]
    return grayscale ** 2

def find_path(src_img_transformed, dst_img, offset_x):
    height_dst, width_dst= dst_img.shape[0], dst_img.shape[1]
    height_src, width_src= src_img_transformed.shape[0], src_img_transformed.shape[1]
    if offset_x>0:
        src_img_overlap=src_img_transformed[:, :width_dst-offset_x]
        dst_img_overlap=dst_img[:, offset_x:]
        row_length=width_dst-offset_x
    else:
        src_img_overlap=src_img_transformed[:, -offset_x:width_dst]
        dst_img_overlap=dst_img[:, :width_dst+offset_x]
        row_length=width_dst+offset_x
...
#         row_length=width_dst-offset_x
#     else:
#         src_img_overlap=src_img_transformed[-offset_y:-offset_y+shape_y, -offset_x:]
...
########## TASK 6 ########## 
############################
def combine_photos(src_img, dst_img, path_matches):
    cv2.imshow("example source image", src_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imshow("example destination image", dst_img)
    ...
    pts1=np.array(pts1)
    pts2=np.array(pts2)
    H13, mask = cv2.findHomography(pts1, pts2, cv2.RANSAC)
    pts1=np.array(pts1)
    pts2=np.array(pts2)
    def draw_keypoints(image, points, color=(0, 0, 255), radius=5, thickness=2):
    ...
    # Assuming `pts1` and `pts2` are the matched points from SuperGlue
    # and `image1` and `image2` are the corresponding images
    pts1 = pts1.reshape(-1, 2)  # Flatten for easier processing
    pts2 = pts2.reshape(-1, 2)

    # Draw keypoints on both images
    image1_with_keypoints = draw_keypoints(src_img, pts1, color=(255, 0, 0))  # Blue markers
    image2_with_keypoints = draw_keypoints(dst_img, pts2, color=(0, 255, 0))  # Green markers

    # Display or save the images
    cv2.imshow("Image 1 Keypoints", image1_with_keypoints)
    cv2.imshow("Image 2 Keypoints", image2_with_keypoints)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    H13=find_homography(pts1, pts2)

    src_img_transformed, min_x, max_x, offset_y, max_y=apply_projective_transformation(src_img, H13)
    #offset_x=min_x
    if min_x>0:
        offset_x=min_x
    else:
        offset_x=min_x
        # offset_x=1280-max_x
        # src_img_transformed=np.fliplr(src_img_transformed)
    ...
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imshow("src img transformed", src_img_transformed)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #best_seam=find_path(src_img_transformed, dst_reshaped, offset_x, offset_y, size[1])
    best_seam=find_path(src_img_transformed, dst_reshaped, offset_x)
    plot_seam(src_img_transformed, dst_reshaped, best_seam, offset_x)
    stitched_img=stitch_images(dst_reshaped, src_img_transformed, best_seam, offset_x)
    cv2.imshow("stitched image", stitched_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imwrite("task_6_stitched.jpg", stitched_img)
    return stitched_img



src_img = cv2.imread(f"./undistorted_stitching/undistorted_img4.png")
dst_img = cv2.imread(f"./undistorted_stitching/undistorted_img3.png")
path_matches = './matches/undistorted_img4_undistorted_img3_matches.npz'
...
# pts1=np.array(pts1)
# pts2=np.array(pts2)
# H57=find_homography(pts1, pts2)
# src_img_transformed, min_x, max_x, offset_y, max_y=apply_projective_transformation(src_img, H57)
# offset_x57=min_x
# dst_reshaped=get_new_shape_dst(dst_img, src_img_transformed, offset_x57, offset_y)



a=combine_photos(src_img, dst_img, path_matches)
# src_img=cv2.imread(f"./undistorted_stitching/undistorted_img5.png")
# path_matches = './matches/undistorted_img5_undistorted_img7_matches.npz'
# combine_photos(src_img, a, path_matches)

# cv2.imwrite("task_5_stitched.jpg", stitched_img)
# cameraMatrix2=np.array([[922.60899902,   0.,         669.2471293 ], [  0.,         923.83355036, 354.36857806], [  0.,           0.,           1.        ]])
# distCoeffs2=np.array([[ 0.15710769, -0.46939228, -0.00242906,  0.00201248,  0.38783533]])
